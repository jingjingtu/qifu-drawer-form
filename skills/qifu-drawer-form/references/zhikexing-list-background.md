...
仅当 `platform=zhikexing` 且 `presentationMode=OVERLAY_ON_ZHIKEXING_LIST` 时启用。本协议不是新的抽屉组合；抽屉仍从 `drawer-compositions.md` 选择 `compositionName`。

`STANDALONE` 是默认值：只创建抽屉，不创建列表页、蒙层或外层 Scene。
...
- 未提供 `background` 时，`TEMPLATE` 为默认来源；若模板不可解析，返回 `BACKGROUND_TEMPLATE_MISSING`，不得临时手绘底图。
...
├── Background / Zhikexing List Page              1366 x 768, whole-frame clone
