...
├── theme-registry.json  # themeKey → collection mode / 颜色变量组 / 绑定策略
...
5. 选择主题时使用 `基础` collection 的真实模式：`毓数/Light` 或 `智能运营/Light`。`mode/green` 与 `mode/blue` 是 `色彩` collection 内的变量路径分组，不是 Figma mode；具体映射以 `theme-registry.json` 为准。
