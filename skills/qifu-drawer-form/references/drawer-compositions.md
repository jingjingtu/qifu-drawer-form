...
与 `qifu-list-page/references/page-rules.md` 中列表页组合注册表的关系:同一份"配方协议"思想在不同原型上的复用;不得把列表页的 6 个组合名搬到抽屉,也不得临时创造第 7 个抽屉组合。
...
| `Qifu Drawer Form / Advanced Create` | 新建，16–20 字段且仍可在单页分区完成 | 3–5 | 单列/双列 | 取消 + 提交 | Data |
...
2. 用户未给名称时按以下优先级选择：含摘要/筛选/内嵌表格的数据详情 → Readonly 需求 → Edit 需求（字段需回填）→ 16–20 字段且可单页分区 → 字段数 7–15 且有分组 → 字段数 ≤ 6 无分组。
3. 多步骤流程、字段超过 20 个或需要跨步骤回退时返回 `UNSUPPORTED_ARCHETYPE`，建议使用 Wizard 或整页表单；不得塞入抽屉。
4. 未指定且无法区分时使用 `Qifu Drawer Form / Sectioned Create`。
5. 组合名称只决定稳定结构(分区框架、按钮组、默认状态);业务字段、文案、控件、数据、`widthTier` 仍由当前 `DrawerSpec` 决定。
6. 不把业务对象、页面名称、状态文案或视觉尺寸拼进 `compositionName`,也不创建 `Basic`、`Normal`、`Default` 等未注册别名。
7. 新场景无法由现有组合表达时,先使用最接近的已注册组合并记录缺口;只有用户确认需要形成长期复用结构后,才新增组合名称并同步本注册表、`SKILL.md` 和验证规则。
8. `Data Detail with Table` 必须完整读取 `golden-sample-data-detail-drawer.md`，并以 680px、16px 内容安全边距、真实组件表格、独立分页器和整数几何为验收门槛。
...
footerButtons[]     cancel | ok | submit | save | close
defaultState        data | loading | disabled | error
```

不得以重命名 Figma 组件的方式实现上述协议。真实组件仍按[组件映射](../../qifu-shared/references/component-map.md)中的正式名称解析。
